...
async def get_interaction_log(call_id: str, db: Session = Depends(get_db)):
    """
...
async def list_interactions(
    ...
    try:
        ...
        interactions = (
            query.order_by(desc(Interaction.created_at))
            .offset(offset)
            .limit(limit)
            .all()
        )
    ...
    try:
        ...
        for channel in ChannelType:
            count = (
                ...
                .filter(
                    Interaction.channel == channel, Interaction.created_at >= start_date
                )
                .count()
            )
        ...
        for status in InteractionStatus:
            count = (
                db.query(Interaction)
                .filter(
                    Interaction.status == status, Interaction.created_at >= start_date
                )
                .count()
            )
        ...
        intent_stats = (
            db.query(Interaction.intent, func.count(Interaction.id).label("count"))
            .filter(
                Interaction.created_at >= start_date, Interaction.intent.isnot(None)
            )
