...
async def process_voice_interaction(webhook_request: VoiceWebhookRequest, db: Session):
    """
    ...
    try:
        ...
        intent_result = await intent_service.extract_intent(
            text=webhook_request.transcription or "", channel=webhook_request.channel
        )
        ...
        await comm_service.send_voice_response(
            to_number=webhook_request.from_number, response_text=response["text"]
        )
        ...
        logger.error(
            "Error processing voice interaction", call_id=call_id, error=str(e)
        )
