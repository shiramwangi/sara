...
class CalendarService:
    ...
    def __init__(self):
        ...
        if not settings.debug:
            ...
            logger.info(
                "DEBUG mode: Skipping Google Calendar authentication (simulated)"
            )
    ...
    def _authenticate(self):
        ...
        try:
            ...
            if (
                ...
                or settings.google_service_account_info
            ):
                try:
                    ...
                    if settings.google_calendar_delegated_user:
                        creds = creds.with_subject(
                            settings.google_calendar_delegated_user
                        )
                    logger.info("Google Calendar authenticated via service account")
                except Exception as e:
                    logger.error(
                        "Service account auth failed, falling back to OAuth client flow",
                        error=str(e),
                    )
                    creds = None
    ...
    async def create_appointment(
        ...
        try:
            if settings.debug:
                ...
                logger.info(
                    "DEBUG mode: Simulating calendar event creation", event_id=fake_id
                )
                return fake_id
            if not self.service:
            ...
            logger.info(
                "Calendar event created", event_id=event_id, start_time=start_datetime
            )
    ...
    async def check_availability(
        self, appointment: AppointmentSlot, duration_minutes: int = 60
    ) -> bool:
        """
    ...
    async def get_available_slots(
        ...
        try:
            ...
            while current_time + timedelta(minutes=duration_minutes) <= end_datetime:
                ...
                has_conflict = False
                for event in events:
                    if event.get("status") == "cancelled" or self._is_event_declined(
                        event
                    ):
                        continue
            ...
            logger.info(
                "Available slots generated", date=date, slots_count=len(available_slots)
            )
    ...
    async def cancel_appointment(self, event_id: str) -> bool:
        ...
        try:
            if settings.debug:
                logger.info(
                    "DEBUG mode: Simulating appointment cancellation", event_id=event_id
                )
                return True
            if not self.service:
                raise Exception("Google Calendar service not initialized")
            ...
            logger.error(
                "Error cancelling appointment", event_id=event_id, error=str(e)
            )
            return False
        except Exception as e:
            logger.error(
    ...
    async def update_appointment(
        ...
        try:
            if settings.debug:
                logger.info(
                    "DEBUG mode: Simulating appointment update", event_id=event_id
                )
                return True
            if not self.service:
                raise Exception("Google Calendar service not initialized")
            ...
            event["summary"] = (
                f"Appointment with {contact_info.name if contact_info and contact_info.name else 'Client'}"
            )
            ...
            logger.info(
                "Appointment updated", event_id=event_id, new_start_time=start_datetime
            )
            return True
    ...
    def _parse_appointment_datetime(self, appointment: AppointmentSlot) -> datetime:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid appointment datetime: {appointment.date} {appointment.time}"
            )

    def _build_event_description(
        self, contact_info: Optional[ContactInfo], additional_description: str = ""
    ) -> str:
    ...
    def _build_attendees(
        self, contact_info: Optional[ContactInfo]
    ) -> List[Dict[str, str]]:
