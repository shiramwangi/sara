...
class CommunicationService:
    ...
    async def send_voice_response(self, to_number: str, response_text: str) -> bool:
        """
        ...
        try:
            ...
            logger.error(
                "Error sending voice response", to_number=to_number, error=str(e)
            )
            return False

    async def send_whatsapp_message(
        self, to_number: str, message_text: str, media_url: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                logger.info(
                    "WhatsApp message sent", to_number=to_number, message_id=message_id
                )
                return True

        except Exception as e:
            logger.error(
                "Error sending WhatsApp message", to_number=to_number, error=str(e)
            )
            return False
    ...
    async def send_sms(self, to_number: str, message_text: str) -> bool:
        ...
        try:
            ...
            message = self.twilio_client.messages.create(
                body=message_text, from_=settings.twilio_phone_number, to=to_number
            )
        ...
        try:
            if response.channel == ChannelType.VOICE:
                return await self.send_voice_response(
                    to_number=response.to_number, response_text=response.text
                )
                ...
                return await self.send_sms(
                    to_number=response.to_number, message_text=response.text
                )
    ...
    async def send_appointment_confirmation(
        ...
        try:
            ...
            message_parts = [
                (
                    f"Hello {contact_info.get('name', 'there')}!"
                    if contact_info
                    else "Hello!"
                ),
                "",
            ...
            if channel == ChannelType.VOICE:
                ...
                subject = (
                    f"Appointment Confirmation - {appointment_details.get('date')}"
                )
    ...
    def generate_twiml_response(self, message_text: str) -> str:
        ...
        try:
            ...
            response = VoiceResponse()
            response.say(
                "Thank you for calling. We'll get back to you soon.", voice="alice"
            )
            response.hangup()
            return str(response)
